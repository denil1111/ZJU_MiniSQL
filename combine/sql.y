...
stmt: select_stmt {
   ...
   | USE DATABASE NAME {$$=new USE_DATABASE_NODE;$$->now_db=std::string($3);}
   | SHOW DATABASES {$$=new SHOW_DATABASE_NODE;$$->show_flag=true;}
   | SHOW TABLES {$$=new SHOW_TABLE_NODE;$$->show_flag=true;}
   | EXIT {Exit exit;throw exit;}
   | QUIT {Exit exit;throw exit;}
   ;
