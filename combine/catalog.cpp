...
void Catalog::create_database(std::string database_name)
{
...
bool Catalog::is_database(std::string database_name)
{
    int i;
...
void Catalog::drop_database(std::string database_name)
{
    int i;
    for (i=0; i < database_list.size(); i++)
    {
        if (database_list[i].database_name==database_name)
        {
            database_list.erase(database_list.begin()+i);
            break;
        }
    }
}
...
Attribute Catalog::drop_index(std::string index_name)
{
    int i,j,k;
    for (i=0; i < database_list.size(); i++)
    {
        for (j=0; j < database_list[i].table_list.size(); j++)
        {
            for (k=0; k < database_list[i].table_list[j].attribute_list.size(); k++)
            {
                if (database_list[i].table_list[j].attribute_list[k].index_name == index_name)
                {
                    ...
                    return database_list[i].table_list[j].attribute_list[k];
                }
            }
        }
    }
...
std::vector<std::string> Catalog::show_database()
{
    ...
    for (i=0; i < database_list.size(); i++)
    {
        databases.push_back(database_list[i].database_name);
    }
    return databases;
}

std::vector<std::string> Catalog::show_table(std::string database_name)
{
    ...
    for (i=0; i < database_list.size(); i++)
    {
        if (database_list[i].database_name==database_name)
        {
            for (j=0; j < database_list[i].table_list.size(); j++)
            {
                tables.push_back(database_list[i].table_list[j].table_name);
            }
            break;
        }
    }
